...
extern NV_ALIGN(64) TRB    TRBRing[4]; //The TRB structure referenced by this physical memory pointer shall be aligned to a 16-byte boundary.
...
NvBootError NvBootXusbMscPrepareCDB(NvBootUsb3Context *Context)
{
    ...
    switch(BufferCBW->CBWTag)
    {
        ...
        case READ10_CMD_OPCODE      :   
                                        //debug this Context->TransferLen== 1? 1<<MSC_MAX_PAGE_SIZE_LOG_2
        ...
        case READ_FORMAT_CAPACITY_CMD_OPCODE
                                      : BufferCBW->CBWDataTransferLength                = 252;//sizeof(ReadFmtCapacityResponse); changing the size to 252
                                        ...
                                        ((ReadFormatCapacityCDB *)BufferCDB)->AllocationLen_msb = (NvU8)((BufferCBW->CBWDataTransferLength) >> 8);
                                        ((ReadFormatCapacityCDB *)BufferCDB)->AllocationLen_lsb = (NvU8)((BufferCBW->CBWDataTransferLength) & 0xff);
                                        break;
        // Not used by bootrom but useful to keep it here in order to make Uartmon implementation easier
        case WRITE10_CMD_OPCODE       : BufferCBW->CBWDataTransferLength                    = 0x200;//*(Context->TransferLen);
...
NvBootError NvBootXusbMscPrepareCBW(NvBootUsb3Context *Context,NvU8 opcode)
{
...
void NvBootXusbPrepareNormalTRB(NvBootUsb3Context *Context,NvU8 *DataBufferPtr, NvU32 TRBTfrLen)
{
...
void NvBootXusbMscBotAnalyzeReceiveData(NvBootUsb3Context *Context, NvU8 opcode)
{
    NvU32 tmp;
    switch(opcode)
    {
...
NvBootError NvBootXusbMscBotReceiveData(NvBootUsb3Context *Context, NvU8 opcode)
{
    NvBootError ErrorCode = NvBootError_Success;
...
NvBootError NvBootXusbMscBotSendCBW(NvBootUsb3Context *Context, NvU8 opcode)
{
...
NvBootError NvBootXusbMscBotProcessRequest(NvBootUsb3Context *Context, NvU8 opcode)
{
    ...
    NvBootUtilMemset((void *)&TRBRing[0],0,sizeof(TRBRing));//To make sure that all reserved bits are set to 0
    while(RetryCount)
    {
	    //Send Command
	    EpType = USB_BULK_OUT;
