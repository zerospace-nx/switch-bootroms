...
__packed typedef struct UsbDeviceDescriptorRec // packing is required to avoid structure padding by compiler
{
    ...
    NvU8    bMaxPacketSize0;               //??h Maximum packet size for endpoint zero. (only 8, 16, 32, or 64
    ...
    NvU8    iSerialNumber;                 //??h Index of string descriptor describing the device’s serial number.
 ...
 * descriptors.
 */
__packed typedef struct UsbConfigDescriptorRec  // packing is required to avoid structure padding by compiler
    {
        NvU8  bLength;
...
__packed typedef struct UsbInterfaceDescriptorRec  // packing is required to avoid structure padding by compiler
    {
        NvU8  bLength;
        NvU8  bDescriptorType;
...
__packed typedef struct UsbEndpointDescriptorRec // packing is required to avoid structure padding by compiler
    {
        NvU8  bLength;
        NvU8  bDescriptorType;
