...
NV_ALIGN(64) TRB    TRBRing[4] = {0}; //The TRB structure referenced by this physical memory pointer shall be aligned to a 16-byte boundary.
NV_ALIGN(16) TRB    TRBDeque;
NV_ALIGN(16) NvU8   BufferXusbCmd[32]   ; // 32 bytes are good enough for MSC BOT commands
NV_ALIGN(16) NvU8   BufferXusbStatus[32]; // 32 bytes are good enough for MSC BOT status
NV_ALIGN(16) NvU8   BufferXusbData[256] ; // 256 bytes are good enough for MSC BOT command response and also good enough for control data TRB during enumeration
...
*
    ...
    USB Boot Port 0x2 =     XUSB Host Controller Port No. 5 (T124 only)                  =          HSPI_PVTPORTSCx2 (T124 only)

    USB Boot Port 0x7 =     XUSB Host Controller Port No. 5 (T132) / No. 6 (T124)     =          HSPI_PVTPORTSCx2 (T132) / =            HSPI_PVTPORTSCx3 (T124)

    USB Boot Port 0x9 =     XUSB Host Controller Port No. 6 (T132) / No. 7 (T124)     =          HSPI_PVTPORTSCx3 (T132) / =            HSPI_PVTPORTSCx4 (T124)
    USB Boot Port 0xA =    XUSB Host Controller Port No. 7 (T132) / No. 8 (T124)     =          HSPI_PVTPORTSCx4 (T132) / =            HSPI_PVTPORTSCx5 (T124)
*
*/
static NvBool BootPortControllerMapping(NvU8 BootPortNumber, NvU8* ControllerPortNumber)
{
...
static NvBootError HwUsbIsDeviceAttached(NvBootUsb3Context *Context )
{
...
{
    volatile NvU32 *HotPlugPtr = (NvU32*)(0x700985f4);
    volatile NvU32 HotPlug, temp = 0;
...
static NvBootError DevicePortInit( NvBootUsb3Context *Context )
{
    NvBootError ErrorCode = NvBootError_Success;
    NvU32 value = 0;
    NvU32 RegOffset = 0;
    ...
    */
    ...
    //skip MFINT programming,, default value holds good.
...
static void InitializeDataStructures(NvBootUsb3Context *Context, NvU8 RootPortNum)
{
    ...
    //flow
    Context->Usb3BitInfo              = &BootInfoTable.SecondaryDevStatus.Usb3Status;
    ...
    Context->TRBRingDequePtr          = (TRB *)(&TRBDeque);
...
static void NvBootXusbUpdateWorkQ(NvBootUsb3Context *Context)
{
    NvU32 RegOffset =0;
    NvU32 value = 0;
...
static NvBootError NvBootXusbCheckCompQ(NvBootUsb3Context *Context)
{
...
static NvBootError ParseConfigurationDescriptor(NvBootUsb3Context *Context)
{
    ...
    UsbInterfaceDescriptorPtr = (UsbInterfaceDescriptor *)( ((NvU8 *)(BufferXusbData)) + UsbConfigDescriptorStructSz );
    ...
    UsbEndpointDescriptorPtr  = (UsbEndpointDescriptor *)(((NvU8 *)UsbInterfaceDescriptorPtr) + UsbInterfaceDescriptorStructSz);
    ...
    for (i = 0; ((i < NumInterfaces) && (ErrorCode != NvBootError_Success)) ; i++)
    {
        ...
        UsbInterfaceDescriptorPtr = (UsbInterfaceDescriptor *)(((NvU8 *)UsbInterfaceDescriptorPtr) + NumEndPoints * UsbEndpointDescriptorStructSz);
            ...
            (UsbInterfaceDescriptorPtr->bInterfaceProtocol == INTERFACE_PROTOCOL_BULK_ONLY_TRANSPORT))
        {
            ...
            for(j = 0; j < NumEndPoints; j++)
            {
                ...
                UsbEndpointDescriptorPtr = (UsbEndpointDescriptor *)((NvU8 *)UsbEndpointDescriptorPtr + UsbEndpointDescriptorStructSz);
            }
        }
        else
        {
...
static NvBootError InitializeScsiMedia(NvBootUsb3Context *Context)
{
    NvBootError e = NvBootError_Success;
...
static NvBootError EnumerateDevice(NvBootUsb3Context *Context)
{
    NvBootError e = NvBootError_Success;
    DeviceRequest DeviceRequestVar;
...
void XusbPadCtrlInit(const NvBootUsb3Params *Params, NvBootUsb3Context *Context)
{
   ...
   if(Params->RootPortNumber == USB_BOOT_PORT_OTG0)
   {
	   ...
	   RegData = NV_FLD_SET_DRF_NUM( XUSB_PADCTL, USB2_BATTERY_CHRG_OTGPAD0_CTL1, VREG_FIX18, 1, RegData);
   ...
   else if(Params->RootPortNumber == USB_BOOT_PORT_OTG1)
   {
	   ...
	   NV_XUSB_PADCTL_WRITE( USB2_PAD_MUX, RegVal);
           ...
           RegData = NV_FLD_SET_DRF_NUM( XUSB_PADCTL, USB2_BATTERY_CHRG_OTGPAD1_CTL1, VREG_FIX18, 1, RegData);
   ...
   else if(Params->RootPortNumber == USB_BOOT_PORT_OTG2)
   {
	   ...
	   RegData = NV_FLD_SET_DRF_NUM( XUSB_PADCTL, USB2_BATTERY_CHRG_OTGPAD2_CTL1, VREG_FIX18, 1, RegData);
   ...
   else if(Params->RootPortNumber == USB_BOOT_PORT_OTG3)
   {
	   ...
	   RegData = NV_FLD_SET_DRF_NUM( XUSB_PADCTL, USB2_BATTERY_CHRG_OTGPAD3_CTL1, VREG_FIX18, 1, RegData);
...
void XusbClkRstInit(NvBootUsb3Context *Context)
{
    ...
    */
    ...
    // Clear SWR_XUSB_HOST_RST field in CLK_RST_CONTROLLER_RST_DEVICES_U_0 
    ...
    //Set NV_PROJ__XUSB_CFG_4 to 0x7009_0000 (Note: bit[14:0] of this register is read only)
    ...
    NvU8 *Dest)
{
	...
	while(RetryCount)
	{
            *((NvU32*)(Context->LogicalBlkAddr)) = ((Block << (Context->BlockSizeLog2 - (Context->PageSizeLog2))) + Page );
            *((NvU16 *)(Context->TransferLen))   = 1;//Context->DeviceNumBlocksPerPage;
...
NvBootError NvBootUsb3GetReaderBuffersBase(NvU8** ReaderBuffersBase,
