...
static const NvU32 IROM_CAM_ADDR_MASK = 0xFFFF0000;

static const NvU32 UINT_BITS = 32 ;
static const NvU32 LOG2_UINT_BITS = 5 ;
static const NvU32 INSIDE_UINT_OFFSET_MASK = 0x1F ;

static const NvU32 H16_BITS = 16 ;
static const NvU32 H16_START_OFFSET = 1 << (16 - 2) ;
static const NvU32 H16_ECC_MASK      = 0xFFF0FFFFU ;
static const NvU32 H16_PARITY_MASK   = 0x00008000U ;
static const NvU32 H16_H_MASK        = 0x00007FFFU ;

static const NvU32 H5_CODEWORD_SIZE = 12 ;
static const NvU32 H5_BIT_OFFSET = 20 ; // the H5 code word is mapped on MSB of a word
static const NvU32 H5_CODEWORD_MASK = 0xFFF00000U ; 
static const NvU32 H5_PARITY_MASK   = 0x01000000U ;

static const NvU32 NO_ERROR = 0 ;
static const NvU32 CORRECTED_ERROR = 1 ;
static const NvU32 UNCORRECTED_ERROR_ODD = 2 ; // calculated syndrome associated with a position outside the buffer itself
static const NvU32 UNCORRECTED_ERROR_EVEN = 3 ;
...
#ifdef __arm__
#pragma arm section rodata = "IPatchNonsecure", code = "IPatchNonsecure"
#endif
...
static const NvU32 H5SyndromeTable[] = { 0x1, 
                                                0x2,
                                                0x4,
                                                0x8,
...
static NvU32 Parity( NvU32 *Data, NvU32 N) {
    NvU32 ParityWord, i ;
    ParityWord = Data[0] ;
    for (i=1; i<N; i++) {
        ParityWord ^= Data[i] ;
...
static NvU32 Hamming5Syndrome(NvU32 *Data) {
  NvU32 i, Syndrome;
...
static NvU32 Hamming5Decode( NvU32 *Data) {
...
static NvU32 NvBootIRomPatchHamming16Syndrome( NvU32 *Data, NvU32 N) {
  NvU32 i, j, Syndrome ;
...
static NvU32 NvBootGetIRomPatchSize() {
    NvU32 RegData;
...
// debug build.
static NvU32 NvBootIRomPatchHamming16Decode( NvU32 *Data, NvU32 N) {
    return (NvU32) 0x0;
}
#else // NV_DEBUG
/*
 ...
 *
 */
static void NvBootIRomPatchCpy4( void *dest, const void *src, size_t size )
{
    int i;
    NvU32 *p = (NvU32 *)src, *q = (NvU32 *)dest;

    NV_ASSERT( !((NvU32)src & 0x03) && !((NvU32)dest & 0x03) && !(size & 0x03) );
...
// Stub out this function to save code space in the non secure section on the
// debug build.
static void NvBootIromPatchParseUpdateCAM(NvU32 * PatchBuff, NvU32 C)
{
    return;
}
#else // NV_DEBUG
...
{
    extern volatile NvU32 SwCYAWord[];
    return SwCYAWord[0];
}
