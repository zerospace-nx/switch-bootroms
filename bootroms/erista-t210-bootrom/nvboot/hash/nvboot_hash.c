...
#define NV_BOOT_HASH_CMAC_RB 0x87
...
#define NV_BOOT_HASH_IS_MSB_SET(x)  ( (x) >> (8*sizeof(x)-1) )
...
NvBootHashGenerateSubkeys (
    ...
    NvBootAesIv *pK1)
{
    NvU8 src[NVBOOT_AES_BLOCK_LENGTH_BYTES];
    NvU32 i;
    NvU32 carry;
    NvU32 msbL;
    NvU8 *p;
    ...
    while ( NvBootAesIsEngineBusy(engine) )
        ;
        
    // set initial vector to all zeroes
    
    NvBootAesClearIv(engine, NVBOOT_HASH_SLOT);
    
    // compute L = AES(key, zeroes); store result in *pK1

    NvBootUtilMemset(src, 0x0, sizeof(src));
    
    NvBootAesStartEngine(engine, 1 /* one block */, src, (NvU8*)pK1->iv,
                         NV_TRUE);
     ...
     */
    if (NvBootAesIsHashingEnabled(engine))
    {
        NvBootAesReadHashOutput(engine, (NvU8 *)pK1->iv);
    }
    ...
    p = (NvU8 *)pK1->iv;
    msbL = NV_BOOT_HASH_IS_MSB_SET(*p);

    p[0] <<= 1;
    for (carry=0, i=1; i<NVBOOT_AES_BLOCK_LENGTH_BYTES; i++)
    {
        carry = NV_BOOT_HASH_IS_MSB_SET(p[i]);
        p[i-1] |= carry;
        p[i] <<= 1;
    }
    ...
    {
        p = (NvU8 *)pK1->iv;
        p[NVBOOT_AES_BLOCK_LENGTH_BYTES-1] ^= NV_BOOT_HASH_CMAC_RB;
    }
    ...
    NvBootHash *pHash)
{
    ...
    if ( first && last ) {
        ...
        // if message is only one block long, load K1 into initial vector
        NvBootAesSetIv(engine, NVBOOT_HASH_SLOT, pK1);

    } else if ( first ) {

        // if first block of multi-block message, set initial vector to zeroes
        NvBootAesClearIv(engine, NVBOOT_HASH_SLOT);

    } else if ( last ) {

        // for last block of multi-block message, load initial vector with
        // "K1 XOR AES(K,M(n-1))", where M(n-1) is the previous data block
        //
        // AES(K,M(n-1)) is in the AES engine's IV buffer, but it's also in
        // the *pHash buffer; furthermore, *pHash has the advantage that it's
        // already in memory

        for (i=0; i<NVBOOT_AES_IV_LENGTH; i++)
            pK1->iv[i] ^= pHash->hash[i];

        NvBootAesSetIv(engine, NVBOOT_HASH_SLOT, pK1);

    }

    // encrypt the input data; the resulting ciphertext is the hash value
    NvBootAesStartEngine(engine, 1 /* one block */, pData, (NvU8 *)pHash->hash,
                         NV_TRUE);
    ...
    if (numBlocks)
    {
        ...
        if(lastChunk)
        {
            if(numBlocks > 1)
            {
                ...
                NvBootAesStartEngine(engine, (numBlocks-1), pData,
            ...
            // encrypt the input data; the resulting
            // ciphertext is the hash value
            NvBootAesStartEngine(engine, 1 /* last block */, (NvU8 *)lastBuffer,
                                 (NvU8 *)pHash->hash, NV_TRUE);
        ...
        else
        {
            ...
            // encrypt the input data; the resulting
            // ciphertext is the hash value
            NvBootAesStartEngine(engine, numBlocks, pData, (NvU8 *)pHash->hash,
                                 NV_TRUE);
        }
