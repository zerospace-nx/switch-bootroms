...
static void DoSdramInit(const NvBootSdramParams *pData, NvBool IsWarmboot)
{
    ...
    NvU32 dpd4_val_e_dpd_vttgen;
    NvU32 dpd3_val;
    NvU32 dpd3_val_sel_dpd;
    NvU32 addr;
    NvU32 rfu_settings, rfu_mask1, rfu_mask2, rfu_step1, rfu_step2;
    NvU32 common_pad_macro_reset_settings, common_pad_macro_mask1, common_pad_macro_step1;
    const NvU32 EmcBase =
...
//     else
    ...
    if (!IsWarmboot) {
        ...
        dpd3_val = (pData->EmcPmcScratch1 & 0x3FFFFFFF) | 
                     NV_DRF_DEF(APBDEV_PMC, IO_DPD3_REQ, CODE, DPD_ON);
        ...
        NvBootUtilWaitUS(pData->PmcIoDpd3ReqWait);
    
        dpd4_val = (pData->EmcPmcScratch2 & 0x3FFFFFFF) | 
                         NV_DRF_DEF(APBDEV_PMC, IO_DPD4_REQ, CODE, DPD_ON);
        // disable e_dpd_vttgen
        dpd4_val_e_dpd_vttgen = (dpd4_val ^ 0x3FFF0000) & 0xFFFF0000;
        NV_WRITE32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_IO_DPD4_REQ_0, dpd4_val_e_dpd_vttgen);
        NvBootUtilWaitUS(pData->PmcIoDpd4ReqWait);
        // disable e_dpd_bg 
        ...
        dpd4_val = (pData->EmcPmcScratch2 & 0x3FFFFFFF) | 
                         NV_DRF_DEF(APBDEV_PMC, IO_DPD4_REQ, CODE, DPD_OFF);
        // disable e_dpd_bg 
        ...
        // disable e_dpd_vttgen
        dpd4_val_e_dpd_vttgen = dpd4_val & 0xFFFF0000;
        NV_WRITE32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_IO_DPD4_REQ_0, dpd4_val_e_dpd_vttgen);
        NvBootUtilWaitUS(pData->PmcIoDpd4ReqWait);
        ...
        dpd3_val_sel_dpd = dpd3_val & 0xC000FFFF;
    ...
    NV_WRITE32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_WEAK_BIAS_0, 0x00000000);
...
//               EmcClkSource_RegData);
    ...
    rfu_settings = NV_RESETVAL(EMC,PMACRO_BRICK_CTRL_RFU1);
    rfu_mask1 = 0x01120112;
    rfu_mask2 = 0x01bf01bf;

    rfu_step1 = rfu_settings & (pData->EmcPmacroBrickCtrlRfu1 | ~rfu_mask1);
    rfu_step2 = rfu_settings & (pData->EmcPmacroBrickCtrlRfu1 | ~rfu_mask2);

    common_pad_macro_reset_settings = NV_RESETVAL(EMC,PMACRO_COMMON_PAD_TX_CTRL);
    common_pad_macro_mask1 = 0x00000001;
    common_pad_macro_step1 = common_pad_macro_reset_settings & (pData->EmcPmacroCommonPadTxCtrl | ~common_pad_macro_mask1);
...
//                    NV_FLD_SET_DRF_DEF(EMC, PIN, PIN_RESET, INACTIVE, pData->EmcPin));
    ...
    HW_REGW(EmcBase, EMC, DLL_CFG_0, pData->EmcDllCfg0);
    HW_REGW(EmcBase, EMC, DLL_CFG_1, pData->EmcDllCfg1);
...
void NvBootSdramInit(const NvBootSdramParams *pData)
{
    ...
    // Set the DDR pad voltage
    Reg = NV_READ32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_DDR_PWR_0);
    COPY_PMC_FLD(DDR_PWR, VAL, PmcDdrPwr);
    NV_WRITE32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_DDR_PWR_0, Reg);
...
NvU32 NvBootSdramQueryTotalMB(void)
{
...
//Bootrom implements it's own version using NvBootSdramQueryTotalMB
