...
NvBootError NvBootRcmSetupPortHandle(NvBootRCMPortID_T RcmPortId)
{
    ...
    {
        sRCMPort.Context.PortId      = RCM_USB_OTG;
        sRCMPort.Init                = (RCMPortInit_T)NvBootUsbfInit;
        sRCMPort.Connect             = (RCMPortConnect_T)SetupUsb;
        sRCMPort.ReceiveStart        = (RCMPortReceiveStart_T)NvBootUsbfReceiveStart;
        sRCMPort.ReceivePoll         = (RCMPortReceivePoll_T)WaitForUsbRecvComplete;
        sRCMPort.Receive             = (RCMPortReceive_T)NvBootUsbfReceive;
        sRCMPort.TransferStart       = (RCMPortTransferStart_T)NvBootUsbfTransmitStart;
        sRCMPort.TransferPoll        = (RCMPortTransferPoll_T)WaitForUsbTxferComplete;
        sRCMPort.Transfer            = (RCMPortTransfer_T)NvBootUsbfTransmit;
        sRCMPort.HandleError         = (RCMPortHandleError_T)HandleErrorUSB;
    }
    else if(RcmPortId == RCM_XUSB)
    {
