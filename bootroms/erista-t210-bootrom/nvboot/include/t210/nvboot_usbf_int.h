...
/**
 * Wrapper function to be registered with RCMConnect.
 * Performs enumeration.
 */
NvBootError      SetupUsb(NvU8 *pCntrlTxferBuffer);

/**
 * Wrapper function to be registered with RCMReceivePoll.
 * Checks for EP status and returns bytes received.
 * Timeout is not used. Poll till complete or error.
 */
NvBootError WaitForUsbRecvComplete(NvU32 *RecvSize, NvU32 TimeoutMs, NvU8 *pCntrlTxferBuffer);

/**
 * Wrapper function to be registered with RCMHandleError.
 * Error handling is limited to stalling Bulk OUT endpoint.
 */
NvBootError HandleErrorUSB(void);

/**
 * Wrapper function to be registered with RCMTransferPoll.
 * Checks for EP status and returns bytes transferred.
 * Timeout is not used. Poll till complete or error.
 */
NvBootError WaitForUsbTxferComplete(NvU32 *TxferSize, NvU32 TimeoutMs, NvU8 *pCntrlTxferBuffer);
