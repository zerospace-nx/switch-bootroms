...
extern NvU8   BufferXusbCmd[32]   ; // 32 bytes are good enough for MSC BOT commands
extern NvU8   BufferXusbStatus[32]; // 32 bytes are good enough for MSC BOT status
extern NvU8   BufferXusbData[256] ; // 256 bytes are good enough for MSC BOT command response and also good enough for control data TRB during enumeration
...
typedef struct InquiryCDBRec
{
    ...
    NvU8    Resvd2                  ;
    NvU8    Resvd3                  ;
    NvU8    AllocationLen           ;       // maximum number of bytes of inquiry data to be returned. should be set to 0x24
    NvU8    Resvd4                  ;
    NvU8    Resvd5[6]                  ;
...
typedef struct Read10CDBRec
{
    ...
    NvU8    Resvd2                  ;
    NvU8    TransferLen_msb         ;       //transfer length field specifies the number of contiguous logical blocks of data
    NvU8    TransferLen_lsb         ;       //to be transferred. value of 0 indicates that no logical blocks are transferred.
    NvU8    Resvd3                  ;
    NvU8    Resvd4[2]                  ;
...
//Whenever an error is reported, host should issue this command to receive the sense data
//describing what caused the error condition. If host issues some other command then sense data is lost
//Sense data is available if an error condition (CHECK CONDITION) is reported or other info is available
//in any field. If device has no other sense data available to return, it returns a sense key of NO SENSE
//and an additional sense code of NO ADDITIONAL SENSE INFORMATION
//If a device returns CHECK CONDITION status for a REQUEST SENSE command, the sense data may be invalid. This
//could occur if a non-zero resvd bit is detected in cmd packet or the device is malfunctioning
//Devices shall be capable of returning at least 18 bytes of data in response to a REQUEST SENSE
//command. If the Allocation Length is 18 or greater, and a Device returns less than 18 bytes of data, the
//Host Computer should assume that the bytes not transferred are zeros. Host Computers can determine how
//much sense data has been returned by examining the Allocation Length parameter in the Command Packet
//and the Additional Sense Length in the sense data. Devices do not adjust the Additional Sense Length to
//reflect truncation if the Allocation Length is less than the sense data available.

//For sense key and additional sense key opcodes, refer to http://www.usb.org/developers/devclass_docs/usbmass-ufi10.pdf
...
typedef struct RequestSenseCDBRec
{
    ...
    NvU8    Resvd1                  :5;
    NvU8    Resvd2                  ;
    NvU8    AllocationLen           ;   //maximum number of bytes of sense data the host can receive
    NvU8    Resvd3[8]               ; 
}__attribute__((packed)) RequestSenseCDB;

typedef struct RequestSenseResponseRec
{
    NvU8    Valid                   :1; //A Valid bit of zero indicates that the Information field is not as defined in this Specification. A Valid bit
    ...
    NvU8    Resvd2                  :1;
    NvU8    SenseKey                :4;
    NvU8    Information[4]          ;  //This field is command-specific; it is typically used by some commands to return a logical block
                                       //address denoting where an error occurred. If this field has a value, the Valid bit shall be set to one
    NvU8    AdditionalSenseLen      ;  //Device sets the value of this field to ten, to indicate that ten more bytes of
...
//Device stores the execution result status of every command block as Sense Data. Sense Data is
...
typedef enum
{
    NO_SENSE            = 0,    //Indicates that there is no specific sense key information to be reported.
                                //This would be the case for a successful command
    RECOVERED_ERROR     = 1,    //Indicates that the last command completed successfully
                                //with some recovery action performed by the device. Details may be
    ...
    MEDIUM_ERROR,               //Indicates that the command terminated with a non-recovered
                                ...
                                //recorded data. This sense key may also be returned if the device is unable to
                                //distinguish between a flaw in the medium and a specific hardware failure
    ...
    ILLEGAL_REQUEST,            //Indicates that there was an illegal parameter in the Command
                                ...
                                //device detects an invalid parameter in the Command Packet, then it shall
                                //terminate the command without altering the medium. If the device detects an
                                //invalid parameter in the additional parameters supplied as data, then the device
    ...
    DATA_PROTECT,               //Indicates that a command that writes the medium was attempted
                                //on a block that is protected from this operation. The write operation was not performed
    ...
    VENDOR_SPECIFIC,            //This sense key is available for reporting vendor specific conditions
    RESERVED1,
...
//Specification is optional.If the device does not have further information related to the error or exception condition, the 
//Additional Sense Code is set to NO ADDITIONAL SENSE INFORMATION.
...
//The TEST UNIT READY command provides a means to check if the Device is ready. This is not a request
//for a self-test. If the Device would accept an appropriate medium-access command without returning
//CHECK CONDITION status, this command returns a GOOD status. If the Device cannot become
//operational or is in a state such that a Host Computer action is required to make the Device ready, the
...
//The TEST UNIT READY command is useful in that it allows a Host Computer to poll a Device until it is
//ready without the need to allocate space for returned data. It is especially useful to check media status.
//Devices are expected to respond promptly to indicate the current status of the Device.
//Only status stage and no data stage
...
//can be formatted on the currently installed medium. If no medium is currently installed, the device shall
...
typedef struct ReadFormatCapacityCDBRec
{
    ...
    NvU8    Resvd1                  :5;
    NvU8    Resvd2[5]               ; 
    NvU8    AllocationLen_msb       ; //specifies the maximum number of bytes of format data the Host can receive. If this is less
                                      //than the size of capacity data, the device returns only the number of bytes requested. However, the
...
//Upon receipt of this command block, the device returns a Capacity List to the host on the Bulk In endpoint.
// No media in FDU (Floppy disk unit): Capacity List Header + Maximum Capacity Header
// Media in FDU (Floppy disk unit): Capacity List Header + Current Capacity Header + Formattable Capacity Descriptors (if any)

//Capacity List Header
typedef struct ReadFmtCapacityRespCapListHdrRec
{
    NvU8    Resvd1[3]                       ;
    NvU8    CapacityListLen                 ;  //specifies the length in bytes of the Capacity Descriptors that follow. Each
                                               //Capacity Descriptor is eight bytes in length, making the Capacity List Length equal to eight times 
...
typedef struct ReadFmtCapacityRespCurCapHdr
{
    union
    {
        ...
        struct
        {
            NvU8    BlockNumInByte_Byte4_msb        ; //Number of Blocks field indicates the total number of addressable blocks for the descriptor’s media type
    ...
    NvU8    Resvd1                          :6;
    NvU8    DescriptorCode                  :2; //specifies the type of descriptor returned to the Host.
    NvU8    BlockLenInByte_Byte3_msb        ;   //specifies the length in bytes of each logical block for the given capacity descriptor.
    NvU8    BlockLenInByte_Byte2            ;
    NvU8    BlockLenInByte_Byte1_lsb        ;  
...
//*******************************************************************
//Note : Assumption for Bootrom code at the stage when read format capacity command
//       is sent : Media is present and formatted !!!!!!
//*********************************************************************
///////////////////////////////////////WRITE(10) Command ///////////////////////////
//The WRITE(10) command requests that the Device write the data transferred by the Host Computer to the
//medium
//The Logical Block Address field specifies the logical block at which the write operation begins.
//The Transfer Length field specifies the number of contiguous logical blocks of data that are transferred. A
//Transfer Length of zero indicates that no logical blocks are transferred. This condition is not considered an
//error and no data is written. Any other value indicates the number of logical blocks that are transferred.
...
//The MODE SENSE command provides a means for a Device to report parameters to the Host Computer. It
//is a complementary command to the MODE SELECT command.
...
//default pipe to clear a STALL condition on the Bulk endpoints and to issue class-specific requests
...
//bCBWLUN field in CBW to designate which logical unit of the device is the
...
{
    NvU32   CBWSignature           ;  //Signature that helps identify this data packet as a CBW. The field shall contain the value 43425355h (little endian)
    NvU32   CBWTag                 ;  //A Command Block Tag sent by the host. The device shall echo the contents of this field back to the
                                       //host in the dCSWTag field of the associated CSW. The dCSWTag positively associates a CSW with the
                                       //corresponding CBW
    NvU32   CBWDataTransferLength  ; //The number of bytes of data that the host expects to transfer on the Bulk-In or Bulk-Out endpoint (as
    ...
    NvU8    CBWCB[16]               ; //The command block to be executed by the device. The device shall interpret the first bCBWCBLength
...
{
    NvU32 CSWSignature             ; //Signature that helps identify this data packet as a CSW. The signature field shall contain the value
                                      //53425355h (little endian), indicating CSW.
    NvU32 CSWTag                   ; //The device shall set this field to the value received in the dCBWTag of the associated CBW.
    NvU32 CSWDataResidue           ; //For Data-Out the device shall report in the dCSWDataResidue the difference between the amount of
    ...
    NvU8 CSWStatus                 ; //bCSWStatus indicates the success or failure of the command. The device shall set this byte to zero if
...
#endif /* #ifndef INCLUDED_NVBOOT_XUSB_MISC_H */
