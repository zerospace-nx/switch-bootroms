...
#ifndef INCLUDED_NVBOOT_USB3_LOCAL_H
#define INCLUDED_NVBOOT_USB3_LOCAL_H
...
#define NUM_EP_CXT  1
#define NUM_TRB_RING 4
#define NUM_BYTES_MSC_CMD 32
#define NUM_BYTES_MSC_STATUS 32
#define NUM_BYTES_MSC_DATA 256

#define EP_CXT_START    (ALIGN_ADDR(NVBOOT_DEV_DS_START, 64))
#define EP_CXT_SIZE     (1 * sizeof(EP))
#define TRB_RING_START  (ALIGN_ADDR(EP_CXT_START+EP_CXT_SIZE, 16))
#define TRB_RING_SIZE   (NUM_TRB_RING * sizeof(TRB))
#define MSC_BOT_CMD_START   (ALIGN_ADDR(TRB_RING_START+TRB_RING_SIZE,16))
#define MSC_BOT_CMD_SIZE    (NUM_BYTES_MSC_CMD)
#define MSC_BOT_STATUS_START    (ALIGN_ADDR(MSC_BOT_CMD_START+MSC_BOT_CMD_SIZE,16))
#define MSC_BOT_STATUS_SIZE (NUM_BYTES_MSC_STATUS)
#define MSC_BOT_DATA_START  (ALIGN_ADDR(MSC_BOT_STATUS_START+MSC_BOT_STATUS_SIZE,16))
#define MSC_BOT_DATA_SIZE   (NUM_BYTES_MSC_DATA)
...
enum { T210_USB_HOST_PORT_OTG0 = 0,
            T210_USB_HOST_PORT_1 = 1,
            T210_USB_HOST_PORT_2 = 2,
            T210_USB_HOST_PORT_3 = 3,
            T210_RESERVED = 0xF};
...
typedef enum  
{
    ...
    */
    ...
    GET_CONFIGURATION           = 8,        /*!<Specifies the get configuration request.*/
...
typedef struct DeviceRequestRec 
{
    union
    {
        ...
        struct
        {
            ...
            NvU32 type                  :2;         /*!<Specifies the type, whether this request goes on device, endpoint, interface or other.*/
            NvU32 dataTransferDirection :1;         /*!<Specifies the data transfer direction.*/
        }stbitfield;
    }bmRequestTypeUnion;
    NvU32 bRequest              :8;         /*!<Specifies the specific request.*/
    NvU32 wValue                :16;        /*!<Specifies the field that varies according to request.*/
    NvU32 wIndex                :16;        /*!<Specifies the field that varies according to request; typically used to pass an index or offset.*/
    NvU32 wLength               :16;        /*!<Specifies the number of bytes to transfer if there is a data state.*/
}DeviceRequest;

struct UsbDeviceDescriptorRec // packing is required to avoid structure padding by compiler
{
    ...
    uint8_t    bDeviceClass;                  //00h Class is specified in the interface descriptor.
    uint8_t    bDeviceSubClass;               //00h Subclass is specified in the interface descriptor.
    uint8_t    bDeviceProtocol;               //00h Protocol is specified in the interface descriptor.
    uint8_t    bMaxPacketSize0;               //??h Maximum packet size for endpoint zero. (only 8, 16, 32, or 64
                                           //are valid (08h, 10h, 20h, 40h)).
    uint16_t   idVendor;                      //????h Vendor ID (assigned by the USB-IF).
    uint16_t   idProduct;                     //????h Product ID (assigned by the manufacturer).
    uint16_t   bcdDevice;                     //????h Device release number in binary-coded decimal.
    uint8_t    iManufacturer;                 //??h Index of string descriptor describing the manufacturer.
    uint8_t    iProduct;                      //??h Index of string descriptor describing this product.
    uint8_t    iSerialNumber;                 //??h Index of string descriptor describing the device’s serial number.
    uint8_t    bNumConfigurations;            //??h Number of possible configurations.
} __attribute__((packed)) ;
typedef struct UsbDeviceDescriptorRec UsbDeviceDescriptor;
 ...
 * descriptors.
 */
struct UsbConfigDescriptorRec  // packing is required to avoid structure padding by compiler
{
    uint8_t  bLength;
...
typedef struct UsbConfigDescriptorRec UsbConfigDescriptor;
enum {  UsbConfigDescriptorStructSz		= 9};

/* USB_DT_INTERFACE: Interface descriptor */
struct UsbInterfaceDescriptorRec  // packing is required to avoid structure padding by compiler
{
    uint8_t  bLength;
    uint8_t  bDescriptorType;
...
typedef struct UsbInterfaceDescriptorRec UsbInterfaceDescriptor;
...
struct UsbEndpointDescriptorRec // packing is required to avoid structure padding by compiler
{
    uint8_t  bLength;
    uint8_t  bDescriptorType;
...
typedef struct UsbEndpointDescriptorRec UsbEndpointDescriptor;
...
#endif /* #ifndef INCLUDED_NVBOOT_USB3_LOCAL_H */
