...
NvBool FT_NONSECURE NvBootQueryRstStatusWarmBootFlag()
{
    return NvBootPmcQueryFlag(NvBootPmcFlagId_Wb0);
}
void
NvBootPmcSetTzramPwrControl(bool ctrl)
{
    uint32_t tzram_pwr_ctrl = NV_READ32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_TZRAM_PWR_CNTRL_0);
    tzram_pwr_ctrl = NV_FLD_SET_DRF_NUM(APBDEV_PMC, TZRAM_PWR_CNTRL, TZRAM_SD, ctrl, tzram_pwr_ctrl);
    NV_WRITE32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_TZRAM_PWR_CNTRL_0, tzram_pwr_ctrl);
}
void
NvBootPmcEnableWriteLockToTzramPwrControl(void)
{
    if(NvBootFuseIsNvProductionMode() == true)
    {
        uint32_t tzram_sd_write_disable = NV_READ32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_TZRAM_SEC_DISABLE_0);
        tzram_sd_write_disable = NV_FLD_SET_DRF_DEF(APBDEV_PMC, TZRAM_SEC_DISABLE, SD_WRITE, ON, tzram_sd_write_disable);
        NV_WRITE32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_TZRAM_SEC_DISABLE_0, tzram_sd_write_disable);
        tzram_sd_write_disable = NV_READ32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_TZRAM_NON_SEC_DISABLE_0);
        tzram_sd_write_disable = NV_FLD_SET_DRF_DEF(APBDEV_PMC, TZRAM_NON_SEC_DISABLE, SD_WRITE, ON, tzram_sd_write_disable);
        NV_WRITE32(NV_ADDRESS_MAP_PMC_BASE + APBDEV_PMC_TZRAM_NON_SEC_DISABLE_0, tzram_sd_write_disable);
    }
}

/**
  * Check if debug spin loop on warmboot error is set and is PreProduction Mode.
 */
NvBool NvBootQueryWarmbootErrorDebugSpin()
{
    NvBool halt_flag = NvBootPmcQueryFlag(NvBootPmcFlagId_HaltAfterWb0);
    if(halt_flag && NvBootFuseIsPreproductionMode())
        return NV_TRUE;
    else
        return NV_FALSE;
}
