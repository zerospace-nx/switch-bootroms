...
{  
	...
	NvBootClocksPllId_UtmiPll = CLK_RST_CONTROLLER_UTMIPLL_HW_PWRDN_CFG0_0,
...
#define NVBOOT_CLOCKS_CHECK_PLLID(PllId) \
    NV_ASSERT( (PllId == NvBootClocksPllId_PllC4) || \
			   ...
			   (PllId == NvBootClocksPllId_UtmiPll) \
) ;
 ...
 */
void FT_NONSECURE NvBootClocksSetEnable(NvBootClocksClockId ClockId, NvBool Enable);
...
/* 
 * NvBootConfigureUsecTimer(): Configure microsecond timer counter.
 */
void FT_NONSECURE
...
/*
 * NvBootClocksEnablePmcClock
 *
 * Function to enable the clock to PMC. Must be called before
 * PMC scratch registers are accessed.
 */
