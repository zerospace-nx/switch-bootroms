...
typedef enum
{
    NvBootStrapDevSel_Emmc = 0x0,                 /* eMMC primary (x8), SDR 51Mhz, Multipage */
    NvBootStrapDevSel_SpiFlash,                   /* SPI Flash             = 0x1 */
    NvBootStrapDevSel_Sata,                      /* Sata            = 0x2 */
    NvBootStrapDevSel_UseFuses,                  /* Use fuses; Only valid when PRODUCTION_MODE fuse is set = 0x3 */
    NvBootStrapDevSel_Usb3,                      /* USB3  = 0x4*/
#if NVENABLE_FOOS_SUPPORT
    NvBootStrapDevSel_Foos,                      /* Foos  = 0x5*/
#endif
    NvBootStrapDevSel_Rcm,                       /* RCM  = 0x5 if not FOOS*/
...
/**
 * Reads back strap value based on the reset status .*
 * @return value from strap or scratch
 */
 ...
 * ups/downs.
 *
 * @returns an uint8_t contains the value 0, 1, 2, or 3, depending on the strap
 *          setting
 */
uint8_t
