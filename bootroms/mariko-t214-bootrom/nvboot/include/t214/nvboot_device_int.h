...
/*
 * NvBootDeviceStatus: The current status of a request.
 */
typedef enum
{
    ...
    NvBootDeviceStatus_WriteInProgress,
    NvBootDeviceStatus_WriteFailure,
 ...
 * the device for reading or writing.  Note that the routine will likely be 
 * called multiple times - once for initially gaining access to the device,
 ...
 *
 * Note: The Read() call cannot occur prior to the Init() for the
 * device.
 */
typedef NvBootError
(*NvBootDeviceRead)(const NvU32 Block, const NvU32 Page, const NvU32 Len, uint8_t *Dest);

/**
 * NvBootDevWrite(): Initiate the Writing of specified size of data 
 * from Dest.
 *
 * @param Block Number of the block from which to write
 * @param Page Number of the page within the block to write into
 * @param Len   Total Len of write data in bytes.
 * @param Dest Storage for the write data for the device.
 *
 * @retval NvBootError_Success No Error
 * @retval NvBootError_InvalidParameter Illegal parameter value
 * @retval NvBootError_DeviceNotResponding
 * @retval NvBootError_DataCorrupted
 * @retval NvBootError_DataUnderflow
 * @retval NvBootError_DeviceWriteError
 * @retval NvBootError_EccFailureCorrected
 * @retval NvBootError_EccFailureUncorrected
 *
 * Note: The Write() call cannot occur prior to the Init() for the
 * device.
 */
typedef NvBootError
(*NvBootDeviceWrite)(const NvU32 Block, const NvU32 Page, const NvU32 Len, uint8_t *Dest);
...
/**
 * NvBootDeviceGetReaderBuffersBase() : Allocated buffers to be used by reader code.
 *
 * @param ReaderBuffersBase pointer to pointer to a valid memory physical address at which
 ...
 *
 */
typedef NvBootError (*NvBootDeviceGetReaderBuffersBase)(uint8_t **ReaderBuffersBase,
                                                        const NvU32 Alignment, const NvU32 Bytes);

/**
 * NvBootDevicePinMuxInit(): Setup the required pinmux.
 *
 * @param Params Pointer to a device-specific parameter structure.
 *
 * @retval NvBootError_Success No Error
 * @retval NvBootError_InvalidParameter Illegal parameter value
 */
typedef NvBootError (*NvBootDevicePinMuxInit)(const void *Params);
