 ...
 * @param Len number of bytes to read. SDMMC driver is responsible for
 *          computing the number of sectors(transfer size) to be read.
...
/**
 * Initiate the writing of a page of data from Dest.buffer.
 * 
 * @param Block Block number to write into.
 * @param Page Page number in the block to write. 
 *          valid range is 0 <= Page < PagesPerBlock.
 * @param Len number of bytes to write. SDMMC driver is responsible for
 *          computing the number of sectors(transfer size) to be written.
 * @param Dest Buffer holding write data.
 *
 * @retval NvBootError_Success Write operation is launched successfully.
 * @retval NvBootError_HwTimeOut Device is not responding.
 * @retval NvBootError_DeviceResponseError Response Recevied from device 
 *          indicated that operation had failed.
 * @retval NvBootError_DeviceError Error in communicating with device. No need
...
/**
 * Allocated buffers to be used by reader code.
 *
 *
...
/**
 * NvBootSdmmcPinMuxInit(): Setup the required pinmux.
 *    SDMMC4 is used for all communication with emmc and no
 *    pinmux required per the customer pinmux sheet.
 *
 *
 * @retval NvBootError_Success No Error
 * @retval NvBootError_InvalidParameter Illegal parameter value
 * @retval NvBootError_None or NvBootError_UnImplemented No implementation provided.
 */
