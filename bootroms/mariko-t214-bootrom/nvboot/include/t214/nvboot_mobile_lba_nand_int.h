...
/**
 * nvboot_mobile_lba_nand_int.h - Public definitions for using MobileLbaNand
 * as the second level boot device.
 */

#ifndef INCLUDED_NVBOOT_MOBILE_LBA_NAND_INT_H
#define INCLUDED_NVBOOT_MOBILE_LBA_NAND_INT_H
 ...
 * 
 * @note NvBootMobileLbaNandInit() must have beeen called atleast once before 
...
/**
 * Uses the data pointed to by DeviceParams to initialize a MobileLbaNand 
...
NvBootMobileLbaNandInit(
    const NvBootMobileLbaNandParams *ParamData,
 ...
 *  API NvBootMobileLbaNandReadPage, if it is pending.
 *
...
/**
 * Shutdowns the MobileLbaNand device and cleansup the state.
 * 
 */
...
#endif /* #ifndef INCLUDED_NVBOOT_MOBILE_LBA_NAND_INT_H */
